...
public class MenuInfo extends JPanel implements ActionListener {
	...
	public MenuInfo() {
		...
		p1_l1 = new JLabel("Please enter food id or food name: ");
		...
		String[] params = { "1" };
		String sql = "select foodid,foodname, price, type from menu where 1 = ?";
		em = new EmpModel();
		em.query(sql, params);
	...
	public void actionPerformed(ActionEvent arg0) {
		if (arg0.getSource().equals(p1_jb)) {
			if (p1_jtf.getText().trim().equals("")) {
				...
				String sql = "select foodid, foodname, price, type from menu where foodid = ? or foodname = ? or type = ?";
				em = new EmpModel();
				em.query(sql, params);
				jtable.setModel(em);
			}
			...
			if (rowNum == -1) {
				...
				new UpdFoodDialog(this, "Edit", true, em, rowNum);
			}
			String[] params = { "1" };
			String sql = "select * from menu where 1 = ?";
			em = new EmpModel();
