...
public class EmpLogin extends JPanel implements ActionListener {
	...
	public EmpLogin() {
		...
		p1_l1 = new JLabel("Please enter employee id: ");
		...
		String[] params = { "1" };
		String sql = "select login.empid,login.empname,passwd,login.position from login, empinfo where login.empid = empinfo.empid and 1 = ?";
		em = new EmpModel();
		em.query(sql, params);
	...
	public void actionPerformed(ActionEvent arg0) {
		if (arg0.getSource().equals(p1_jb)) {
			if (p1_jtf.getText().trim().equals("")) {
				...
				String sql = "select login.empid, login.empname, passwd, login.position from login, empinfo where login.empid = empinfo.empid and login.empid = ?";
				em = new EmpModel();
				em.query(sql, params);
				jtable.setModel(em);
			}
			...
			String sql = "select * from empinfo where empinfo.empid = ?";
			String[] params = { empId };
			em = new EmpModel();
			em.query(sql, params);
			jtable.setModel(em);
			...
			String sql = "select empid, empname, passwd, position from login where 1 = ?";
			String[] params = { "1" };
			em = new EmpModel();
			em.query(sql, params);
			jtable.setModel(em);
			...
			if (rowNum == -1) {
				...
				new UpdLoginDialog(this, "Edit", true, em, rowNum);
			}
			String sql = "select empid, empname, passwd, position from login where 1 = ?";
			String[] params = { "1" };
			em = new EmpModel();
